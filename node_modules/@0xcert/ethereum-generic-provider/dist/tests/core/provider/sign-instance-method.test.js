"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const spec_1 = require("@specron/spec");
const __1 = require("../../..");
const types_1 = require("../../../core/types");
const spec = new spec_1.Spec();
spec.before((stage) => __awaiter(this, void 0, void 0, function* () {
    const accounts = yield stage.web3.eth.getAccounts();
    const provider = new __1.GenericProvider({
        client: stage.web3,
        signMethod: types_1.SignMethod.ETH_SIGN,
        accountId: accounts[0],
    });
    stage.set('provider', provider);
}));
spec.only('signs a message', (ctx) => __awaiter(this, void 0, void 0, function* () {
    const provider = ctx.get('provider');
    const signature = yield provider.sign('test');
    ctx.true(!!signature);
}));
exports.default = spec;
//# sourceMappingURL=sign-instance-method.test.js.map
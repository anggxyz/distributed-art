...
spec.test('checks if account is operator', (ctx) => __awaiter(this, void 0, void 0, function* () {
    const xcert = ctx.get('protocol').xcert;
    const bob = ctx.get('bob');
    const coinbase = ctx.get('coinbase');
    const ledger = ctx.get('ledger');
    yield xcert.instance.methods.setApprovalForAll(bob, true).send({ from: coinbase });
    ctx.true(yield ledger.isApprovedOperator(coinbase, bob));
}));
spec.test('checks if gateway proxy is operator', (ctx) => __awaiter(this, void 0, void 0, function* () {
    const xcert = ctx.get('protocol').xcert;
    const ledger = ctx.get('ledger');
    const gateway = ctx.get('gateway');
    const coinbase = ctx.get('coinbase');
    const proxyId = ctx.get('protocol').nftokenSafeTransferProxy.instance.options.address;
    yield xcert.instance.methods.setApprovalForAll(proxyId, true).send({ from: coinbase });
    ctx.true(yield ledger.isApprovedOperator(coinbase, gateway));
}));
spec.test('return null when checking contract that does not support operators', (ctx) => __awaiter(this, void 0, void 0, function* () {
    const bob = ctx.get('bob');
    const coinbase = ctx.get('coinbase');
    ...
    ctx.is(yield ledger.isApprovedOperator(coinbase, bob), null);
}));
exports.default = spec;
//# sourceMappingURL=is-approved-operator-instance-method.test.js.map
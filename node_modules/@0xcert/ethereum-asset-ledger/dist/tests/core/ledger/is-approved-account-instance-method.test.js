...
spec.test('checks if account is approved', (ctx) => __awaiter(this, void 0, void 0, function* () {
    const coinbase = ctx.get('coinbase');
    const bob = ctx.get('bob');
    const ledger = ctx.get('ledger');
    ctx.false(yield ledger.isApprovedAccount('1', bob));
    yield ledger.approveAccount('1', bob);
    ctx.true(yield ledger.isApprovedAccount('1', bob));
    yield ledger.approveAccount('1', coinbase);
    ctx.false(yield ledger.isApprovedAccount('1', bob));
}));
spec.test('checks if gateway proxy is approved', (ctx) => __awaiter(this, void 0, void 0, function* () {
    const coinbase = ctx.get('coinbase');
    const ledger = ctx.get('ledger');
    const gateway = ctx.get('gateway');
    ctx.false(yield ledger.isApprovedAccount('2', gateway));
    yield ledger.approveAccount('2', gateway);
    ctx.true(yield ledger.isApprovedAccount('2', gateway));
    yield ledger.approveAccount('2', coinbase);
    ctx.false(yield ledger.isApprovedAccount('2', gateway));
}));
exports.default = spec;
//# sourceMappingURL=is-approved-account-instance-method.test.js.map